//nolint:revive // ignore all revive issues her
package main
...
var ErrEnvVarRequired = fmt.Errorf("required environment variable not set")
...
func checkEnvVar(envVar string, required bool) (string, error) {
	...
	if envVarValue == "" && required {
		...
		return "", fmt.Errorf("%w: %s", ErrEnvVarRequired, envVar)
	}
	if envVarValue == "" {
